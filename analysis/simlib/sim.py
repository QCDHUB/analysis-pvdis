...
def gen_conf(wdir,idxs,tar,exp='idis',option=1,fn=None):
    ...
    for idx in idxs:
        conf['datasets'][exp]['xlsx'][idx]='./%s/sim/%d.xlsx'%(wdir,idx)
    ...
    if exp=='idis':
        ...
        if option ==  1: 
            ...
            conf['pidis grid']  = {}
            conf['pidis grid']['overwrite'] = True
...
def update_tabs(wdir,tabs,tar,exp):
...
def get_tables(wdir,idxs,tar,exp,fn,bins):
    central = {}
    replicas= {}
    for idx in idxs:
        replicas[idx] = []
    ...
    else: X,Q2=get_xQ2_grid(option=0)

    return X,Q2,central,replicas

def gen_lhapdf_dat_file(wdir,idxs,tar,setlabel=0,dirname='JAMSIM',option=0,exp=None,fn=None,bins=True):

    #--get tables
    X,Q2,central,replicas=get_tables(wdir,idxs,tar,exp,fn,bins)
    nx=len(X)
    nQ2=len(Q2)
...
def gen_lhapdf_info_file(wdir,idxs,tar,info,dirname='JAMSIM',exp=None,fn=None,bins=False):

    #--get tables
    X,Q2,table,replicas=get_tables(wdir,idxs,tar,exp,fn,bins)
...
def A_PV_e_errors(wdir,idxs,tar,lum='100:fb-1',kind='optimistic'):

    for idx in idxs:
        load_config('%s/input.py'%wdir)
        conf['aux'] = AUX()
        conf['eweak'] = EWEAK()
        data = pd.read_excel('%s/sim/%s.xlsx'%(wdir,idx), index=False)
        data = data.to_dict(orient='list')
        ...
        idis.data[target]['F2g']  = np.zeros(idis.X.size)
        idis.data[target]['FLg']  = np.zeros(idis.X.size)
        idis.data[target]['F2gZ'] = np.zeros(idis.X.size)
        idis.data[target]['FLgZ'] = np.zeros(idis.X.size)
        idis.data[target]['F3gZ'] = np.zeros(idis.X.size)
        if tar == 'd':
        ...
        par = np.mean(replicas,axis=0)
        parman.set_new_params(par,initial=True)
        idis._update()

        F2g  = lambda x,q2: idis.get_stf(x,q2,stf='F2g' ,tar=target) 
        FLg  = lambda x,q2: idis.get_stf(x,q2,stf='FLg' ,tar=target) 
        F2gZ = lambda x,q2: idis.get_stf(x,q2,stf='F2gZ',tar=target) 
        FLgZ = lambda x,q2: idis.get_stf(x,q2,stf='FLgZ',tar=target) 
        ...
        if kind == 'optimistic':
            data['syst_u'] = np.array(data['value'])*0.00
        #--moderate: 1% below y = 0.01, 1.5% above 
        elif kind == 'moderate':
            data['syst_u'] = np.zeros(l)
            for i in range(l):
                if Y[i] <= 0.01: data['syst_u'][i] = data['value'][i]*0.010
                if Y[i] >  0.01: data['syst_u'][i] = data['value'][i]*0.015
        #--pessimistic: do not have yet
        elif kind == 'pessimistic':
            return

        else:
            print('kind must be optimistic, moderate, or pessimistic')
            return
...
def A_PV_had_errors(wdir,idxs,tar,lum='100:fb-1',kind='optimistic'):
    ...
    for idx in idxs:
        ...
        idis  = resman.idis_thy
        pidis.data[target]['g1gZ'] = np.zeros(pidis.X.size)
        pidis.data[target]['g5gZ'] = np.zeros(pidis.X.size)
        idis.data[target]['F2g']  = np.zeros(idis.X.size)
        idis.data[target]['FLg']  = np.zeros(idis.X.size)
        idis.data[target]['F2gZ'] = np.zeros(idis.X.size)
        idis.data[target]['FLgZ'] = np.zeros(idis.X.size)
        ...
        g1gZ = lambda x,q2: pidis.get_stf(x,q2,stf='g1gZ',tar=target) 
        g5gZ = lambda x,q2: pidis.get_stf(x,q2,stf='g5gZ',tar=target) 
        F2g  = lambda x,q2: idis.get_stf(x,q2,stf='F2g'  ,tar=target) 
        FLg  = lambda x,q2: idis.get_stf(x,q2,stf='FLg'  ,tar=target) 
        F2gZ = lambda x,q2: idis.get_stf(x,q2,stf='F2gZ' ,tar=target) 
        FLgZ = lambda x,q2: idis.get_stf(x,q2,stf='FLgZ' ,tar=target) 
        ...
        #--moderate: 2% below y = 0.01, 2.5% above 
            ...
            data['syst_u'] = np.zeros(l)
            for i in range(l):
                if Y[i] <= 0.01: data['syst_u'][i] = data['value'][i]*0.020
                if Y[i] >  0.01: data['syst_u'][i] = data['value'][i]*0.025
        #--pessimistic: do not have yet
        elif kind == 'pessimistic':
            return
