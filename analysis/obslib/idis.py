...
def plot_proton(wdir, data, kc, istep):
    ...
    py.tight_layout()
    py.savefig('%s/gallery/dis-proton-%d.png' % (wdir, istep), dpi = 1000)
...
def plot_deuteron(wdir, data, kc, istep):
    ...
    py.savefig('%s/gallery/dis-deuteron-%d.png' % (wdir, istep), dpi = 1000)
    print('Saving figure to %s/gallery/dis-deuteron-%d.png'%(wdir,istep))
    py.close()

def plot_hera_318(wdir, data, kc, istep):
    data_hera = {}
    try:
    ...
    py.savefig('%s/gallery/dis-hera-318-%d.png' % (wdir, istep), dpi = 1000)
    print('Saving figure to %s/gallery/dis-hera-318-%d.png'%(wdir,istep))
    py.close()

def plot_hera_other_than_318(wdir, data, kc, istep):
    data_hera = {}
    try:
    ...
    py.savefig('%s/gallery/dis-hera-other-%d.png' % (wdir, istep), dpi = 1000)
    print('Saving figure to %s/gallery/dis-hera-other-%d.png'%(wdir,istep))
    py.close()

def plot_obs(wdir, kc, plot_HERA = True):
    ...
    if 'idis' not in predictions['reactions']:
        sys.exit('inclusive DIS is not in data file')
    ...
    if plot_HERA:
        plot_hera_318(wdir, data, kc, istep)
        plot_hera_other_than_318(wdir, data, kc, istep)
